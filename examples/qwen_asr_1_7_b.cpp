...
static const float* audio_position_row(int position)
{
    ...
    if (table.empty())
    {
        table.resize(qwen_audio_max_tokens * qwen_audio_hidden_size);
        for (int p = 0; p < qwen_audio_max_tokens; p++)
        {
            float* row = table.data() + p * qwen_audio_hidden_size;
            for (int c = 0; c < qwen_audio_hidden_size; c++)
                row[c] = audio_position_value(p, c);
        }
    }

    return table.data() + position * qwen_audio_hidden_size;
}
